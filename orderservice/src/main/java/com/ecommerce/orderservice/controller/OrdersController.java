...
public class OrdersController {
    ...
    private OrderdetailsDTO mapOrderDetails(Orderdetails order){
        OrderdetailsDTO res= new OrderdetailsDTO(order.getOrder_detail_id(), order.getProduct_id(),order.getQuantity(),order.getName(),order.getImage(),order.getUnit_price());
        return res;
    }
    ...
    public ResponseEntity<?> saveToCart(
        @Valid @RequestBody ProductCartDTO product)
    {
        String uri = "http://localhost:8082/products/product/"+product.product_id+"/units";
        ...
        if(availableUnits>=product.quantity){
            uri = "http://localhost:8082/products/product/"+product.product_id;
            restTemplate = new RestTemplate();
            ProductDTO productInfo=restTemplate.getForEntity(uri, ProductDTO.class).getBody();
            product.unit_price=productInfo.getPrice();
            product.image=productInfo.getImage();
            product.name=productInfo.getName();
                                       ...
                                       Long order_id)
    {
        ...
        for(Orderdetails item:aux){
            final String uri = "http://localhost:8082/products/update/product/"+item.getProduct_id()+"/refill/"+item.getQuantity();
            RestTemplate restTemplate = new RestTemplate();
    ...
    completeOrder(@PathVariable("id") Long userID)
    {
        ...
        for(Orderdetails item:aux){
            final String uri = "http://localhost:8082/products/update/product/"+item.getProduct_id()+"/unitsbuy/"+item.getQuantity();
            RestTemplate restTemplate = new RestTemplate();
            restTemplate.exchange(uri, HttpMethod.PUT, null, Object.class);
        }
    ...
    {
        final String uri = "http://localhost:8082/products/product/"+productID+"/units";
        RestTemplate restTemplate = new RestTemplate();
        int availableUnits=restTemplate.getForEntity(uri, Integer.class).getBody();
        long order_id=ordersService.viewCart(userID);
        List<Orderdetails> aux = ordersService.fetchOrdersDetailList(userID, order_id);
